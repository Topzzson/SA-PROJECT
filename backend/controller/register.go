...
func CreateRegister(c *gin.Context) {
	...
	// 12: ค้นหา officer ด้วย id
	if tx := entity.DB().Where("id = ?", register.CaseID).First(&officer); tx.RowsAffected == 0 {
...
func GetRegister(c *gin.Context) {
	...
	if err := entity.DB().Preload("AssessmentSheet").Preload("Ambulance").Preload("Case").Preload("Officer").Raw("SELECT * FROM registers WHERE id = ?", id).Find(&register).Error; err != nil {
...
func ListRegisters(c *gin.Context) {
	...
	if err := entity.DB().Preload("AssessmentSheet").Preload("Ambulance").Preload("Case").Preload("Officer").Raw("SELECT * FROM registers").Find(&registers).Error; err != nil {
