...
func CreateCheckList(c *gin.Context) {
	...
	if tx := entity.DB().Where("id = ?", check_lists.AmbulanceID).First(&ambulance); tx.RowsAffected == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "AmbulanceID not found"})
		return
	}
	// 10:ค้นหา owner ด้วย id
	if tx := entity.DB().Where("id = ?", check_lists.OperatorID).First(&operator); tx.RowsAffected == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "OwnerID not found"})
		return
	}
	// 11:ค้นหา path_status ด้วย id
	if tx := entity.DB().Where("id = ?", check_lists.Path_statusID).First(&path_status); tx.RowsAffected == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Path_statusID not found"})
		return
	}
	// 12:ค้นหา car_path ด้วย id
...
func GetCheckList(c *gin.Context) {
	var check_lists entity.CheckList
	id := c.Param("id")
	if err := entity.DB().Preload("Car_path").Preload("Ambulance").Preload("Path_status").Preload("Operator").Raw("SELECT * FROM check_lists WHERE id = ?", id).Find(&check_lists).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func ListCheckList(c *gin.Context) {
	var check_lists []entity.CheckList
	if err := entity.DB().Preload("Ambulance").Preload("Car_path").Preload("Path_status").Preload("Operator").Raw("SELECT * FROM check_lists").Find(&check_lists).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"data": check_lists})
