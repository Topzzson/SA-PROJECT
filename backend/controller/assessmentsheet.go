...
func CreateAssessmentSheet(c *gin.Context) {
	...
	// 10: ค้นหา recorder ด้วย id
	if tx := entity.DB().Where("id = ?", assessmentsheet.CaseID).First(&recorder); tx.RowsAffected == 0 {
...
func GetAssessmentSheet(c *gin.Context) {
	...
	if err :=
		entity.DB().Preload("Case").Preload("Symptom").Preload("State").Preload("Assess").Preload("Recorder").Raw("SELECT * FROM assessment_sheets WHERE id = ?", id).Find(&assessmentsheet).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func ListAssessmentSheet(c *gin.Context) {
	...
	if err :=
		entity.DB().Preload("Case").Preload("Symptom").Preload("State").Preload("Assess").Preload("Recorder").Raw("SELECT * FROM assessment_sheets").Find(&assessmentsheet).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"data": assessmentsheet})
