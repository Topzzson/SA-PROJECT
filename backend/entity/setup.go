...
func SetupDatabase() {
	...
	db.Model(&Operator{}).Create(&Operator{
		Name:     "Rattatammanoon",
	...
	//--User
	var rattatammanoon Operator
	var oppo Operator
	db.Raw("SELECT * FROM users WHERE email = ?", "rattatammanoontop@gmail.com").Scan(&rattatammanoon)
	db.Raw("SELECT * FROM users WHERE email = ?", "oppo@gmail.com").Scan(&oppo)
	...
	var jame Officer
	var san Officer
	db.Raw("SELECT * FROM officers WHERE email = ?", "jame@gmail.com").Scan(&jame)
	db.Raw("SELECT * FROM officers WHERE email = ?", "san@gmail.com").Scan(&san)
